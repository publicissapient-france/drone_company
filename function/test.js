...
test('receiving PARCEL_DELIVERED event', async t => {
  let msgSent;
  const index = proxyquire.load(
    './index', {
      './pubsubUtils' : {
  ...
  eventMsg.event = 'PARCEL_DELIVERED';
  const req = {
    body: {
      ...eventMsg
    }
...
test('receiving DESTINATION_REACHED event', async t => {
  let msgSent;
  const index = proxyquire.load(
    './index', {
      './pubsubUtils' : {
  ...
  eventMsg.event = 'DESTINATION_REACHED';
  const req = {
    body: {
      ...eventMsg
    }
